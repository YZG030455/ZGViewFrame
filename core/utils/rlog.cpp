...
bool RLog::init(LogConfig &logConfig)
{
    ...
    if(Core::RUtil::createDir(logDir))
    {
        QString fileName = logDir + QDir::separator() + Core::RUtil::getDate() + QString(Suffix);
...
void RLog::log(RLOG_LEVEL nLevel, const char *fileDesc, const char *functionDesc, int lineNum, const char* data, ...)
{
    ...
    {
        QString recordInfo = QString("[%1]").arg(Core::RUtil::getTimeStamp());
